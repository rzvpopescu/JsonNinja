<template>
  <span :class="getValueClass(data)">
    ...
    <template v-if="isObject">
      ...
      <template v-if="keys.length > 0">
        <div v-for="(key, index) in keys" :key="key" class="ml-4">
          <span class="text-json-key-light dark:text-json-key-dark">"{{ key }}"</span
          ><span class="text-json-brace dark:text-json-brace">:</span>
          <span class="ml-1">
            <SyntaxHighlighter :data="(data as Record<string, unknown>)[key]" />
          </span>
    ...
    <template v-else-if="isString">
      <span class="text-json-string">"{{ data }}"</span>
    </template>

    <!-- Number -->
    <template v-else-if="isNumber">
      <span class="text-json-number">{{ data }}</span>
    </template>

    <!-- Boolean -->
    <template v-else-if="isBoolean">
      <span class="text-json-boolean">{{ data }}</span>
    </template>

    <!-- Null -->
    <template v-else>
      <span class="text-json-null">null</span>
    </template>
  </span>
</template>
